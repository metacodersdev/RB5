...
(function () {
  ...
  Djblets.Forms.ListEditView = Backbone.View.extend({
    ...
    render() {
      ...
      $("input[name=\"".concat(this._fieldName, "_num_rows\"]")).val(this._numItems);
    ...
    _createDefaultEntry(index) {
      ...
      $("input[name=\"".concat(this._fieldName, "_num_rows\"]")).val(index + 1);
      $entry.find('.djblets-c-list-edit-widget__add-item').on('click', e => this._addItem(e));
      return $entry;
    },
    ...
    _updateEntryInputName($entry, index) {
      const $inputs = $entry.find('.djblets-c-list-edit-widget__input'); // The entry may have more than one "input".

      if ($inputs.length > 1) {
        $inputs.each((idx, el) => $(el).attr('name', "".concat(this._fieldName, "_value[").concat(index, "]_").concat(idx)));
      } else {
        $inputs.attr('name', "".concat(this._fieldName, "_value[").concat(index, "]"));
      }
    },
