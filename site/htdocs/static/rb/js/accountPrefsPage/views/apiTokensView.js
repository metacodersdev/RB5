...
(function () {
  ...
  const APITokenItem = RB.Config.ResourceListItem.extend({
    ...
    syncAttrs: ['expires', 'id', 'invalidReason', 'invalidDate', 'lastUsed', 'note', 'policy', 'tokenValue', 'valid'],
  ...
  const PolicyEditorView = Backbone.View.extend({
    ...
    render() {
      ...
      this.$el.modalBox({
        ...
        buttons: [$('<input type="button"/>').val(gettext("Cancel")).click(_.bind(this.cancel, this)), $('<input type="button" class="save-button"/>').val(gettext("Save and continue editing")).click(() => {
          this.save(false);
          return false;
        }), $('<input type="button" class="btn primary save-button"/>').val(gettext("Save")).click(() => {
          this.save(true);
          return false;
        })]
      });
  ...
  const APITokenItemView = Djblets.Config.ListItemView.extend({
    ...
    template: _.template(gettext("<div class=\"rb-c-config-api-tokens__main\"> <div class=\"rb-c-config-api-tokens__value\"> <input readonly=\"readonly\" value=\"<%- tokenValue %>\"> </div> <span class=\"fa fa-clipboard js-copy-token\" title=\"Copy to clipboard\"></span> </div> <div class=\"rb-c-config-api-tokens__info\"> <% if (expired) { %> <p> Expired <time class=\"timesince\" datetime=\"<%= expires %>\"></time> <time datetime=\"<%= expires %>\"></time> </p> <% } else if (valid) { %> <% if (lastUsed) { %> <p> Last used <time class=\"timesince\" datetime=\"<%= lastUsed %>\"></time> </p> <% } else { %> <p>Never used.</p> <% } %>\n\n<% if (expires) { %> <p> Expires <time class=\"timesince\" datetime=\"<%= expires %>\"></time> </p> <% } %> <% } else { %> <p> Invalidated <time class=\"timesince\" datetime=\"<%= invalidDate %>\"></time>: <%= invalidReason %> </p> <% } %> </div> <div class=\"rb-c-config-api-tokens__actions\"></div> <span class=\"rb-c-config-api-tokens__note\"></span>")),
    events: {
    ...
    render() {
      ...
      this._$note = this.$('.rb-c-config-api-tokens__note').inlineEditor({
        editIconClass: 'rb-icon rb-icon-edit'
      }).on({
        beginEdit: () => this._$note.inlineEditor('setValue', this.model.get('note')),
